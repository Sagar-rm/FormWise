...
import { ArrowUpRight, Users, FileText, Eye, CheckCircle, Download, ChevronDown, TrendingUp, Clock } from "lucide-react"
...
const Analytics = () => {
  ...
  useEffect(() => {
    const fetchAnalyticsData = async () => {
      ...
      try {
        ...
        // Fetch responses within date range in batches (Firestore 'in' limit is 10)
        const batchSize = 10
        const batches = []
        ...
        const dailyResponses = days.map((day) => {
          ...
          const dayResponses = allResponses.filter((r) => format(r.submittedAt, "yyyy-MM-dd") === dayStr)

          return {
            date: dayStr,
            responses: dayResponses.length,
            views: Math.round(dayResponses.length * (Math.random() * 3 + 2)), // Simulate views
          }
        })
        ...
        const todayResponses = allResponses.filter(
          (r) => format(r.submittedAt, "yyyy-MM-dd") === format(today, "yyyy-MM-dd"),
        )
        ...
        const deviceChartData = Object.entries(devices).map(([name, value]) => ({
          name,
          value,
          percentage: ((value / allResponses.length) * 100).toFixed(1),
        }))
        ...
        setConversionData({
          rate: totalViews > 0 ? ((totalResponses / totalViews) * 100).toFixed(1) : 0,
          views: totalViews,
        ...
        const formPerformance = forms.map((form) => {
          ...
          const conversionRate = form.views ? ((formResponses.length / form.views) * 100).toFixed(1) : 0
  ...
  const exportAnalytics = () => {
    const data = {
      summary: {
        ...
        activeForms: analytics?.activeForms || 0,
  ...
  // Calculate stats
  const totalForms = analytics?.totalForms || 0
  const totalResponses = analytics?.totalResponses || 0
  const activeForms = analytics?.activeForms || 0
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h3 className="text-2xl font-bold text-gray-800 mt-1">{totalForms}</h3>
                </div>
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h3 className="text-2xl font-bold text-gray-800 mt-1">{totalResponses}</h3>
                </div>
            ...
            >
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h3 className="text-2xl font-bold text-gray-800 mt-1">{totalViews}</h3>
                </div>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {topForms.map((form) => (
                      <tr key={form.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3">{form.responses}</td>
                        ...
                        <td className="px-4 py-3">
                          <Link to={`/form-responses/${form.id}`} className="text-blue-600 hover:text-blue-800 text-sm">
                            View Responses
                          </Link>
                        </td>
                      </tr>
